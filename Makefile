...
TOP_PATH = .
KERNEL_LIST_PATH = ./kernel_list
USER_LIST_PATH = ./user_list
...
KERNEL_LIST_SRC = $(wildcard $(KERNEL_LIST_PATH)/*.c)
KERNEL_LIST_OBJ = $(patsubst %.c, %.o, $(KERNEL_LIST_SRC))

USER_LIST_SRC = $(wildcard $(USER_LIST_PATH)/*.c)
USER_LIST_OBJ = $(patsubst %.c, %.o, $(USER_LIST_SRC))

# 所有目标文件的集合(根据项目情况修改)
TARGET_OBJS := $(TOP_OBJ) $(KERNEL_LIST_OBJ) $(USER_LIST_OBJ)
